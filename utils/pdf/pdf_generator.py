...
class PDFGenerator:
    ...
    def generate_transform_report(self, data, filepath):
        """Generate a PDF report for transform calculations
        ...
        try:
            ...
            transform_type = data.get('transform_type', 'Transform')
            story.append(Paragraph(f"{transform_type} Transform Analysis", title_style))
            story.append(Spacer(1, 12))
            ...
            # Function and parameters table
            params_data = [
                ["Parameter", "Value"],
                ["Transform Type", data.get('transform_type', 'N/A')],
                ["Function Type", data.get('function_type', 'N/A')],
                ["Original Equation", data.get('equation_original', 'N/A')],
                ["Parameter A", f"{data.get('parameter_a', 0):.2f}"],
                ["Parameter B", f"{data.get('parameter_b', 0):.2f}" if data.get('needs_parameter_b', False) else "N/A"],
                ["Frequency", f"{data.get('frequency', 0):.2f} Hz" if data.get('needs_frequency', False) else "N/A"],
                ["Sample Points", f"{data.get('sample_points', 0)}"]
            ]
            
            # Add window type for Fourier transform
            if data.get('transform_type') == "Fourier":
                params_data.append(["Window Function", data.get('window_type', 'None')])
            ...
            story.append(Paragraph("Transform Details", heading_style))
            story.append(Spacer(1, 12))
            
            # Transform equation
            transform_equation = data.get('equation_transform', 'N/A')
            story.append(Paragraph("Transform Equation:", styles["Heading2"]))
            story.append(Paragraph(transform_equation, normal_style))
            story.append(Spacer(1, 12))
            
            # Resonant frequency for Laplace transform
            if data.get('transform_type') == "Laplace" and data.get('resonant_frequency', -1) > 0:
                resonant_freq = data.get('resonant_frequency', 0)
                freq_hz = resonant_freq / (2 * 3.14159)
                story.append(Paragraph("Resonant Frequency:", styles["Heading2"]))
                story.append(Paragraph(f"{resonant_freq:.2f} rad/s ({freq_hz:.2f} Hz)", normal_style))
                story.append(Spacer(1, 12))
            
            # Add charts if provided
            if 'time_domain_image_path' in data and os.path.exists(data['time_domain_image_path']):
                story.append(Paragraph("Time Domain Signal", styles["Heading2"]))
                img = Image(data['time_domain_image_path'])
                img.drawWidth = 450
                img.drawHeight = 250
                story.append(img)
                story.append(Spacer(1, 12))
            
            if 'transform_image_path' in data and os.path.exists(data['transform_image_path']):
                story.append(Paragraph(f"{transform_type} Transform Result", styles["Heading2"]))
                img = Image(data['transform_image_path'])
                img.drawWidth = 450
                img.drawHeight = 250
                story.append(img)
            ...
            story.append(Spacer(1, 24))
            story.append(Paragraph("Mathematical Background", heading_style))
            story.append(Spacer(1, 12))
            
            if data.get('transform_type') == "Fourier":
                story.append(Paragraph("Fourier Transform", styles["Heading2"]))
                ...
                story.append(Spacer(1, 6))
                story.append(Paragraph(
                    "Applications include signal processing, filter design, communications systems, "
                    "and audio analysis.",
                    normal_style
                ))
            else:  # Laplace
                story.append(Paragraph("Laplace Transform", styles["Heading2"]))
                story.append(Paragraph(
                    "The Laplace Transform converts a time-domain function into a complex frequency "
                    "domain function. It is defined as:",
                    normal_style
                ))
                story.append(Spacer(1, 6))
                story.append(Paragraph(
                ...
                story.append(Spacer(1, 6))
                story.append(Paragraph(
                    "Applications include solving differential equations, control systems engineering, "
                    "circuit analysis, and system stability analysis.",
                    normal_style
                ))
